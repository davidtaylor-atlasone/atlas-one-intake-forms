...
- Job 4 skipped: no LibreOffice on this machine (soffice not found, no LibreOffice.app); PowerPoint via AppleScript tried once with a 120 s watchdog: PowerPoint launched and took the open command, then every AppleEvent timed out (-1712), same as Run U. The PowerPoint process my script started was closed; the deck reopens fine (43 slides). No PDFs written, Hub cards unchanged. Script kept at _briefs/assets/run-V/pptx2pdf.applescript.
- Job 5 done: Portal rebuilt with the path argument, BUILD Sep 12, 2026 4:10 PM, 38 tools, 10.35 MB, both patched calculators inlined. Report written to _BUILD-LOG/RUN-V-report.md (copy in _briefs/assets/run-V/). Run complete.
